...
#%%
def branching(prob, node, angle, coords, min_branch, rad_termin, lstep=1, xmin=0, xmax = 300, ymin=0, ymax=300):
    """
    ...
    xmax : float, optional
        Simulation box right boundary. The default is 300.
    ymin : float, optional
        Simulation box bottom boundary. The default is 0.
    ymax : float, optional
        Simulation box top boundary. The default is 300.
    ...
    for j in range(len(prob_diff)):
        ...
        if prob_diff[j]<=0:
            # determine two random branching angles between np.pi/10 and np.pi/2 (note that we can change these min/max angles!):
            ang_branch1 = np.random.uniform(min_branch,np.pi/2)
            ang_branch2 = np.random.uniform(min_branch,np.pi/2)   
            ...
            if len(excluded_coords)>0:
                ...
                tip_distances = distance(excluded_coords,node[j+skip-skipp])
                tip_distances2 = distance(excluded_coords,node[j+skip-skipp+1])
...
#%%
def guidance_avoidance(node, angle, coord_last, coord_until, fc, fs, radavoid):
    """
    ...
    for j in range(len(node)):
        if fc!=0:
            ...
            # For the case of external guidance along an axial field (positive y-direction):
